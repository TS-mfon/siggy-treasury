...
- **CREATE2 Targeting & EIP-7715 Bypass:** Since MetaMask's `wallet_requestExecutionPermissions` API restricts the `from` parameter to EOA addresses owned directly by the wallet (throwing `"Requested address not found"` for counterfactual smart wallets), we bypassed this by using the Hybrid Smart Account's `signDelegation` method. This signs the raw `Delegation` struct with the owner EOA's private key via MetaMask's standard `signTypedData` API. The signature is then serialized and encoded into the required `context` hex string using `@metamask/delegation-core`'s `encodeDelegations` utility and registered to GenLayer. This ensures the smart account address is correctly reconciled throughout the EIP-7715/EIP-7710 pipeline.
